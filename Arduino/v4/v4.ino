...
float lon;
String latitude;
String longitude;  //Storing the Longitude

// Metal detector post management
float met_lat = 0;
float met_lon = 0;


//MQ-137 post manahement
float mq_lat = 0;
float mq_lon = 0;
...
void loop() {
  ...
  float comparator = ppm * 1000;                  // value to be used in if statement comparison
  ...
  if (voltage >= 3) {
    ...
    if (gps.location.isValid()) {
      ...
      if((met_lat!=lat) && (met_lon!=lon)){
        String latitude = String(lat, 6);
        String longitude = String(lon, 6);

        //update post management
        met_lat = lat;
        met_lon = lon;
